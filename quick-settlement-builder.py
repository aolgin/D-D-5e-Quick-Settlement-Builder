...
class settlement:
	# Filled constructor
	def __init__(self, name="", size="", population=0, location="", 
	...
	def generate_settlement(self, debug=False):
		name = "Some settlement"
		if debug:
			print('Name: ' + name)
		#name = tables.first_name_element[self.roll(len(tables.first_name_element))] + tables.second_name_element[self.roll(len(tables.second_name_element))]
		...
		return settlement(name, size, population, location, purpose, resources, races, gods)
...
def __main__():
	...
	if num_settlements <= 0:
		...
		for i in range(0, num_settlements):
			print(settlement_instance.generate_settlement(debug))
	exit()
